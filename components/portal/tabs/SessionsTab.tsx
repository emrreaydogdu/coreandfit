                            ...
                            <div className="h-full rounded-xl p-2.5 bg-gradient-to-br from-[#0F172A] to-slate-800 text-white flex flex-col justify-between shadow-xs">
                              ...
                              <div className="flex items-center justify-between w-full pt-1">
                                <button
                                  ...
                                  onClick={() => {
                                    downloadIcsFile({
                                      title: `Core & Fit: ${session.focusArea} (1:1 İlker Yüksel)`,
                                      description: `Birebir Antrenör Seansı. Lokasyon: Nişantaşı Özel İstasyon.`,
                                      location: "Core & Fit Nişantaşı Studio, İstanbul",
                                      startDate: session.date,
                                      timeSlot: session.timeSlot,
                                    });
                                  }}
                                  className="text-[9px] font-bold text-emerald-300 hover:text-emerald-100 transition-colors"
                                  title="Apple / Google Takvime Ekle"
                                >
                                  + Takvim
                                </button>
                                ...
                                <button
                                  ...
                                  className="text-[9px] font-bold text-rose-300 hover:text-rose-100 transition-colors"
                                >
                                  İptal Et
                                </button>
                              </div>
                  ...
                  <div className="flex items-center gap-2 w-full md:w-auto justify-end pt-3 md:pt-0 border-t md:border-t-0 border-black/[0.06] flex-wrap">
                    <button
                      ...
                      onClick={() => {
                        downloadIcsFile({
                          title: `Core & Fit: ${session.focusArea} (1:1 İlker Yüksel)`,
                          description: `Birebir Antrenör Seansı. Lokasyon: Nişantaşı Özel İstasyon. Not: ${session.notes || "Randevu onaylandı."}`,
                          location: "Core & Fit Nişantaşı Studio, İstanbul",
                          startDate: session.date,
                          timeSlot: session.timeSlot,
                        });
                      }}
                      className="px-3 py-1.5 text-xs font-semibold text-[#0F172A] bg-white hover:bg-slate-50 border border-black/[0.08] rounded-xl transition-all shadow-2xs flex items-center gap-1.5"
                      title="Apple / Google / Outlook Takvimine Aktar (.ics)"
                    >
                    ...
                    <a
                      href={`https://wa.me/905327993427?text=${encodeURIComponent(
                        `Merhaba İlker Hocam, ${session.date} ${session.timeSlot} seansımız ile ilgili yazıyorum.`
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 text-xs font-semibold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/60 rounded-xl transition-all flex items-center gap-1.5"
                      title="Antrenör İlker Yüksel ile WhatsApp üzerinden iletişime geçin"
                    >
                    ...
                    <div className="px-2.5 py-1.5 bg-[#F1F5F9] text-[#0F172A] rounded-xl text-[11px] font-bold">
                      1:1 Özel
                    </div>
                  </div>
                </div>
