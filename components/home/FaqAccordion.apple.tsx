...
export const FaqAccordion: React.FC<FaqAccordionProps> = ({
  ...
  return (
    <section className="py-20 lg:py-28 bg-[#F5F5F7] dark:bg-[#0A0A0C] border-t border-black/[0.06] dark:border-white/[0.08] relative overflow-hidden">
            ...
            {displayItems.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className={cn(
                    "rounded-[26px] transition-all duration-300 overflow-hidden",
                    isOpen
                      ? "liquid-glass-panel shadow-md"
                      : "liquid-glass-card hover:bg-white/60 dark:hover:bg-white/[0.06]"
                  )}
                >
                  <button
                    onClick={() => toggle(idx)}
                    className="w-full flex items-center justify-between text-left gap-4 p-6 focus:outline-none group select-none"
